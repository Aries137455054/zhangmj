$(function() {
	...
	var tableObj = commonUtil.initTable({
        ...
        columns: [
            ...
            {field: 'email',align: 'center', title: '邮箱地址',width:200},
            {field : 'status', title : '状态',align: 'center',width:80,
            	formatter : function(value) {
            			return value == 1 ? "<span class='label label-success'>开启</span>" : "<span class='label label-danger'>禁用</span>";
            		}
            },
	...
	var setRole = function() {
     ...
     // 发送请求
	    $.ajax({
			"type" : "GET",
			"url" : "/authority-shiro/role/getRoleListWithSelected/"+list[0].id,
			"dataType" : "json",
				...
				if (resp.code == 200) {
					...
					layer.open({
				    	  ...
				    	  yes: function(index, layero){
				        	    ...
				        	    $.ajax({
				    				"type" : "POST",
				    				"url" : "/authority-shiro/user/setRole",
...
//				    					}
	...
	var addData = function() {
		window.location.href = "/authority-shiro/user/saveUI";
	}
	...
	var updateData = function() {
		...
		window.location.href = "/authority-shiro/user/saveUI?id="+list[0]["id"];
	}
	...
	var deleteData = function() {
		...
		layer.confirm("确定要删除该数据吗?", {
			...
			$.ajax({
				"type" : "GET",
				"url" : "/authority-shiro/user/delete/" + ids.join(","),
				"dataType" : "json",
				"success" : function(resp) {
					if (resp.code == 200) {
						layer.close(index);
